...
void solve()
{
    ...
    Convex con = convexhull(pts);
    if(con.p.size() <= 2)
    {
        if(con.p.size() <= 1)
            cout << 0 << " " << 1 << "\n";
        else
            cout << con.p[0].id << " " << con.p[1].id << "\n";
        return;
    }
    i64 maxdis = 0;
    pii pair = {-1, -1};
    auto get = [&](const Point &pi, const Point &nxti, const Point &pj) -> void
    {
        i64 d1 = pi.dis2(pj);
        i64 d2 = nxti.dis2(pj);

        if(d1 > maxdis)
            maxdis = d1, pair = {pi.id, pj.id};
        if(d2 > maxdis)
            maxdis = d2, pair = {nxti.id, pj.id};
    };
