...
//注意，这里的v，p要求一定是常量，才能在编译时就处理完
//这种写法最强大的地方在于：它能自动生成。 如果你代码里用到了 CInv<2, 998244353>、CInv<3, 998244353> 和 CInv<2, 1000000007>，编译器会帮你生成三个不同的常量。如果你没用到，它就一个字节都不占。
template<int V, int P>
constexpr MInt<P> CInv = MInt<P>(V).inv();

//写using Z = MInt<P>; 且 P = 998244353 时，编译器会生成一套专门为 998244353 服务的一套代码。这就好比你根据一个 42 号的鞋模子，直接注塑出了一双鞋，这双鞋的大小在出厂（编译）时就固定了。这是全局固定一个模数
//这样子在预编译时就全部处理好了，速度极快
constexpr int P = 998244353;
using Z = MInt<P>;

//如果想在程序运行过程中，根据输入调整 P
//使用 MInt<0>： 你在代码中可以看到 template<> int MInt<0>::Mod = 998244353;。这里的 0 是一个特殊标记，代表“我不使用固定模数，我要用那个静态变量 Mod”。
//在 main 函数里动态设置：
// int main() {
//     int input_p;
//     cin >> input_p;
//     MInt<0>::setMod(input_p); // 动态修改运行时的模数

//     using Z = MInt<0>; // 此时所有的 Z 运算都会模 input_p
//     Z a = 10, b = 20;
//     cout << a + b << endl; }