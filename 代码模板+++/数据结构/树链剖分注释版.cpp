#include <bits/stdc++.h>
using namespace std;
using i64 = long long;

int P; // 全局取模数
//详见typora
// ---------------- 线段树板子开始 ----------------
struct SegmentTree {
    int n;
    vector<i64> sum, lazy;
    
    SegmentTree(int n_) : n(n_), sum(4 * n + 1, 0), lazy(4 * n + 1, 0) {}

    void pull(int p) {
        sum[p] = (sum[p << 1] + sum[p << 1 | 1]) % P;
    }

    void apply(int p, int l, int r, i64 v) {
        sum[p] = (sum[p] + v * (r - l + 1)) % P;
        lazy[p] = (lazy[p] + v) % P;
    }

    void push(int p, int l, int r) {
        if (lazy[p]) {
            int mid = (l + r) >> 1;
            apply(p << 1, l, mid, lazy[p]);
            apply(p << 1 | 1, mid + 1, r, lazy[p]);
            lazy[p] = 0;
        }
    }

    void build(int p, int l, int r, const vector<i64>& a) {
        if (l == r) {
            sum[p] = a[l] % P;
            return;
        }
        int mid = (l + r) >> 1;
        build(p << 1, l, mid, a);
        build(p << 1 | 1, mid + 1, r, a);
        pull(p);
    }

    void build(const vector<i64>& a) {
        build(1, 1, n, a);
    }

    void rangeAdd(int p, int l, int r, int x, int y, i64 v) {
        if (l > y || r < x) return;
        if (l >= x && r <= y) {
            apply(p, l, r, v);
            return;
        }
        push(p, l, r);
        int mid = (l + r) >> 1;
        rangeAdd(p << 1, l, mid, x, y, v);
        rangeAdd(p << 1 | 1, mid + 1, r, x, y, v);
        pull(p);
    }

    void rangeAdd(int x, int y, i64 v) {
        v %= P;
        rangeAdd(1, 1, n, x, y, v);
    }

    i64 rangeQuery(int p, int l, int r, int x, int y) {
        if (l > y || r < x) return 0;
        if (l >= x && r <= y) return sum[p];
        push(p, l, r);
        int mid = (l + r) >> 1;
        return (rangeQuery(p << 1, l, mid, x, y) + rangeQuery(p << 1 | 1, mid + 1, r, x, y)) % P;
    }

    i64 rangeQuery(int x, int y) {
        return rangeQuery(1, 1, n, x, y);
    }
};
// ---------------- 线段树板子结束 ----------------

// ---------------- 树链剖分板子开始 ----------------
struct HLD {
    int n, root, timer;
    vector<vector<int>> adj;
    vector<int> sz, dep, fa, son, top, dfn, rnk;
    vector<i64> val, mapped_val; // val: 原始点权, mapped_val: 按 dfn 排序后的点权
    SegmentTree seg;

    HLD(int n_, int r_) : n(n_), root(r_), timer(0), 
                          adj(n_ + 1), sz(n_ + 1, 0), dep(n_ + 1, 0), 
                          fa(n_ + 1, 0), son(n_ + 1, 0), top(n_ + 1, 0), 
                          dfn(n_ + 1, 0), rnk(n_ + 1, 0), 
                          val(n_ + 1, 0), mapped_val(n_ + 1, 0),
                          seg(n_) {}

    void addEdge(int u, int v) {
        adj[u].push_back(v);
        adj[v].push_back(u);
    }

    void dfs1(int u, int p, int d) {
        dep[u] = d;
        fa[u] = p;
        sz[u] = 1;
        int max_sz = -1;
        for (int v : adj[u]) {
            if (v == p) continue;
            dfs1(v, u, d + 1);
            sz[u] += sz[v];
            if (sz[v] > max_sz) {
                max_sz = sz[v];
                son[u] = v; // 记录重儿子
            }
        }
    }

    void dfs2(int u, int t) {
        dfn[u] = ++timer;
        rnk[timer] = u;
        top[u] = t;
        mapped_val[timer] = val[u]; // 根据 dfn 重新映射点权

        if (!son[u]) return;
        // 优先遍历重儿子，保证重链上的 dfn 是连续的
        dfs2(son[u], t);
        
        // 遍历轻儿子
        for (int v : adj[u]) {
            if (v != fa[u] && v != son[u]) {
                dfs2(v, v); // 轻儿子的 top 是它自己
            }
        }
    }

    void init() {
        dfs1(root, 0, 1);
        dfs2(root, root);
        seg.build(mapped_val); // 用重新映射后的点权建树
    }

    // 树上路径修改
    void modifyPath(int u, int v, i64 w) {
        w %= P;
        while (top[u] != top[v]) {
            // 谁所在的重链顶端深，谁往上跳
            if (dep[top[u]] < dep[top[v]]) swap(u, v);
            seg.rangeAdd(dfn[top[u]], dfn[u], w);
            u = fa[top[u]];
        }
        // 在同一条重链上，此时深度小的 dfn 小
        if (dep[u] > dep[v]) swap(u, v);
        seg.rangeAdd(dfn[u], dfn[v], w);
    }

    // 树上路径查询
    i64 queryPath(int u, int v) {
        i64 res = 0;
        while (top[u] != top[v]) {
            if (dep[top[u]] < dep[top[v]]) swap(u, v);
            res = (res + seg.rangeQuery(dfn[top[u]], dfn[u])) % P;
            u = fa[top[u]];
        }
        if (dep[u] > dep[v]) swap(u, v);
        res = (res + seg.rangeQuery(dfn[u], dfn[v])) % P;
        return res;
    }

    // 子树修改
    void modifySubtree(int u, i64 w) {
        w %= P;
        // u 的子树区间就是 [dfn[u], dfn[u] + sz[u] - 1]
        seg.rangeAdd(dfn[u], dfn[u] + sz[u] - 1, w);
    }

    // 子树查询
    i64 querySubtree(int u) {
        return seg.rangeQuery(dfn[u], dfn[u] + sz[u] - 1);
    }
};
// ---------------- 树链剖分板子结束 ----------------

void solve() {
    int n, m, r;
    cin >> n >> m >> r >> P;

    HLD hld(n, r);

    for (int i = 1; i <= n; i++) {
        cin >> hld.val[i];
    }

    for (int i = 1; i < n; i++) {
        int u, v;
        cin >> u >> v;
        hld.addEdge(u, v);
    }

    hld.init(); // 一定要记得 init 初始化树剖和线段树！

    while (m--) {
        int op;
        cin >> op;
        if (op == 1) {
            int x, y; i64 z;
            cin >> x >> y >> z;
            hld.modifyPath(x, y, z);
        } else if (op == 2) {
            int x, y;
            cin >> x >> y;
            cout << hld.queryPath(x, y) << "\n";
        } else if (op == 3) {
            int x; i64 z;
            cin >> x >> z;
            hld.modifySubtree(x, z);
        } else if (op == 4) {
            int x;
            cin >> x;
            cout << hld.querySubtree(x) << "\n";
        }
    }
}

signed lyc_fan_club() {
    ios::sync_with_stdio(0);
    cin.tie(0);
    int T = 1;
    // cin >> T;
    while(T--)
        solve();
    return 0;
}