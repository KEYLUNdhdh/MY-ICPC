...
struct convex : polygon<T>
{
    ...
    int is_in(const point<T> &a, point<T> &l, point<T> &r)
    {
        const auto &p = this->p;
        int n = p.size();
        if(p.size() == 0)
        ...
        {
            if(a == p[i])
                l = p[this->pre(i)], r = p[this->nxt(i)];
            else
                l = p[i], r = p[this->nxt(this->nxt(i))];
            return -2;
        }
        if(i == 1)
        {
            if(segment<T>{p[0], p[i]}.is_on(a) == 1)
            {
                l = p[0], r = p[i];
                return -1;
            }
...
void solve()
{
    ...
    Convex convex = convexhull(pts);
    int q;
    cin >> q;

    const auto &hp = convex.p;
    ld dis = INF;
    while(q--)
    {
        Point a, b;
        ...
        Segment seg{a, b};
        if(convex.relation(seg) != 1)
        {
            cout << fixed << setprecision(10) << a.dis(b) << "\n";
            continue;
        }

        auto [x, y] = convex.tangent(a);
        auto [p, q] = convex.tangent(b);


        Line a1, a2, b1, b2;
        Point tmp;
        Point l, r;
        if(convex.is_in(a, l, r) < 0)
        {
            a1 = Line{a, l - a};
            a2 = Line{a, r - a};
        }
        ...
        if(!abs(a1.v ^ b1.v) <= eps)
        {
            tmp = a1.inter(b1);
            chmin(dis, tmp.dis(a) + tmp.dis(b));
        }

        if(!abs(a1.v ^ b2.v) <= eps)
        {
            tmp = a1.inter(b2);
            chmin(dis, tmp.dis(a) + tmp.dis(b));
        }

        if(!abs(a2.v ^ b1.v) <= eps)
        {
            tmp = a2.inter(b1);
            chmin(dis, tmp.dis(a) + tmp.dis(b));
        }

        if(!abs(a2.v ^ b2.v) <= eps)
        {
            tmp = a2.inter(b2);
            chmin(dis, tmp.dis(a) + tmp.dis(b));
        }

        if(fabsl(dis - INF) < eps)
            cout << -1 << "\n";
        else
