...
void solve()
{
    ...
    auto check = [&](ld theta) -> ld
    {
        ld x = x3 + r * cosl(theta);
        ld y = y3 + r * sinl(theta);
        Point tmp{x, y};
        return tmp.dis(a) + tmp.dis(b);
    };
    ...
    for (int i = 0; i < 4;i++)
    {
        ...
        for (int k = 0; k < 100;k++)
        {
            ld lmid = left + (right - left) / 3.;
            ld rmid = right - (right - left) / 3.;
            if(check(lmid) < check(rmid))
                right = rmid;
            else
                left = lmid;
        }
        chmin(ans, check(left));
    }
