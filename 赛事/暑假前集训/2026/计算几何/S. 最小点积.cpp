...
void solve()
{
    ...
    for (int i = 0; i < pts.size();i++)
    {
        ...
        {
            int idx = mp[a] - 1;
            int j = convex.nxt(idx);
            int k = -1;
            i64 cur = 1;
            for(int pos = 0; pos < len;pos++)
            {
                if((p[j] - a) * (p[pos] - a) < cur)
                    k = pos, cur = (p[j] - a) * (p[pos] - a);
            }

            chmin(ans, cur);

            for (; j != idx;j = convex.nxt(j))
            {
                const Point &b = p[j];
                Point c = p[k];
                Point nx = p[convex.nxt(k)];
                while ((b - a) * (c - a) > (b - a) * (nx - a))
                {
                    k = convex.nxt(k);
                    c = p[k];
                    nx = p[convex.nxt(k)];
        ...
        else
        {
            ...
            for (int cnt = 0; cnt < len;j = convex.nxt(j), cnt++)
            {
                const Point &b = p[j];
                Point c = p[k];
                Point nx = p[convex.nxt(k)];
