...
/* 算法8-1: 求单源最短路的Dijkstra算法 Dijkstra (graph, s, path, dist) */
void Dijkstra (MGraph graph, Vertex s, Vertex path[], GElemSet dist[])
{
    /* TODO: 请在这里补全算法逻辑。
...
int main(void)
{
	...
	graph = BuildGraph();
	path = (Vertex *)malloc(sizeof(Vertex) * graph->n_verts);
